...
// Create Nodemailer Gmail Transporter if credentials are provided
...
const FROM = process.env.GMAIL_USER ? `Studioflow <${process.env.GMAIL_USER}>` : 'Studioflow <onboarding@resend.dev>';

class UnifiedEmailService implements IEmailService {
  ...
  async sendVerificationEmail(email: string, verificationLink: string): Promise<void> {
    ...
    const htmlContent = `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 20px;">
        <h1 style="color: #a88aed; margin-bottom: 16px;">Studioflow</h1>
        <p style="font-size: 16px; margin: 24px 0; color: #333;">Welcome! Please verify your email address to get started.</p>
  ...
  async sendPasswordResetEmail(email: string, resetLink: string): Promise<void> {
    ...
    const htmlContent = `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 20px;">
        <h1 style="color: #a88aed; margin-bottom: 16px;">Studioflow</h1>
        <p style="color: #333;">Someone requested a password reset for your account.</p>
...
export const emailService: IEmailService = new UnifiedEmailService();
