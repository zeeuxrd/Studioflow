...
export async function POST(request: Request) {
  try {
    ...
    if (product_id && refinement_prompt) {
      ...
      const existingProduct = await prisma.productDefinition.findUnique({
        where: { product_id },
      ...
      const { object } = await generateObject({
        ...
        Current Price Suggestion: $${existingProduct.monetization_price_suggestion}
        
        Refinement Instructions: ${refinement_prompt}
        
        Revise the title, content structure (chapters/modules/checklist items), and suggest an updated price.`,
        schema: z.object({
          ...
          monetization_price_suggestion: z.number().min(0).describe("Updated USD price suggestion.")
        })
      });
